...
async function memberget(btn){
    ...
    if(write){
        if(membercopy!="none"){
            ...
            for(let i=0;i<tableData[0].length;i++)
                send+=`?${tableData[0][i]}?${tableData[1][i]}`
            console.log(send)
            //await fetch(`https://script.google.com/macros/s/AKfycbw-hfbqj7e3YcRLyuim2K6jKPI3qiVrOjLCAJ_LZwPCBm6r_EO-4wagYvtZiiY8D1kR/exec?data=${usercopy.value}?${versioncopy.value}?${unitcopy.value}${send}`);
        }
    }
    //テーブル削除
    ...
    .then(data=>{
        ...
        for(let i=0;i<data.num.length;i++)
            string+=`<tr><td>${data.ver[i]}</td><td><img src="../img/${data.ver[i]}/${member}.jpg"></td><td>${data.num[i]}</td><td><button onclick="dec(${i})">-</button>　<button onclick="add(${i})">+</button></td></tr>`;
        string+=`</table>`;
